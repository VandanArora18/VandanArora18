...
Features:
  - High-res Floyd-Steinberg dithered portrait from photo
  - Morphing transitions: Portrait → </> → {AI} → repeat
...
CW, CH = 260, 330          # pixel-art canvas size
N_PG   = 15                # portrait animation groups
N_SG   = 8                 # symbol groups
FI_T0  = 0.15              # first group begin (s)
FI_DT  = 0.035             # interval between groups
FI_DUR = 0.85              # each group fade duration
CYC_T  = 3.0               # when cycling starts
CYC_D  = 12.0              # cycle period (s)
    ...
    for y in range(h):
        for x in range(w):
            if bw_img.getpixel((x, y)) == 0:
                gs[(x*7 + y*13 + random.randint(0, n-2)) % n].append((x, y))
    return gs
...
def make_symbol(text):
    ...
    for sz in range(220, 40, -8):
        f = _font(sz)
        bb = draw.textbbox((0, 0), text, font=f)
        tw, th = bb[2] - bb[0], bb[3] - bb[1]
        if tw < CW * 0.85 and th < CH * 0.65:
            break
...
def build_svg(mode, p_svg, s1_svg, s2_svg):
    dk = mode == "dark"
    # Colors
    C = {
    ...
    sy = round(420 / CH, 4)
    sfx = "" if dk else "L"
    kt = "0;0.31;0.34;0.64;0.67;0.97;1"
    ca = f'dur="{CYC_D}s" begin="{CYC_T}s" repeatCount="indefinite"'
...
<g clip-path="url(#wc{sfx})">
  ...
  <!-- ═══ Pixel Art with Morphing Animation ═══ -->
  <g transform="translate(48,96) scale({sx},{sy})" fill="url(#ag{sfx})" shape-rendering="crispEdges">

    <!-- PORTRAIT: progressive reveal, then cycles -->
    <g>
      <animate attributeName="opacity" values="1;1;0;0;0;0;1" keyTimes="{kt}" {ca}/>
      {p_svg}
    </g>

    <!-- SYMBOL 1: &lt;/&gt; code brackets -->
    <g opacity="0">
      <animate attributeName="opacity" values="0;0;1;1;0;0;0" keyTimes="{kt}" {ca}/>
      {s1_svg}
    </g>

    <!-- SYMBOL 2: AI -->
    <g opacity="0">
      <animate attributeName="opacity" values="0;0;0;0;1;1;0" keyTimes="{kt}" {ca}/>
      {s2_svg}
    </g>
  </g>
  ...
  <g transform="translate(460,75)">
    ...
    <g transform="translate(0,262)">
      ...
      <rect x="220" y="0" width="105" height="32" rx="6" fill="{C['tb']}" stroke="{T['cv'][1]}"/><text x="232" y="20" font-size="12" fill="{T['cv'][0]}" font-weight="600">👁️ OpenCV</text>
    ...
    <g transform="translate(0,356)">
      ...
      <g transform="translate(20,44)" font-size="12">
        <text x="0" y="15" fill="{C['pc1']}" font-weight="bold">🔹 Carrier Lock System</text><text x="180" y="15" fill="{C['pds']}">Automated carrier locking security system for mobile devices</text>
        <text x="0" y="42" fill="{C['pc2']}" font-weight="bold">🔹 AdmitOne App</text><text x="180" y="42" fill="{C['pds']}">Smart authentication &amp; ticketing platform with Flutter &amp; Firebase</text>
      </g>
    </g>
  </g>
</g>
...
def main():
    ...
    print("1/4  Processing photo (high-res dithering)...")
    pg = process_photo(photo)

    print("2/4  Generating </> symbol...")
    s1 = make_symbol("</>")

    print("3/4  Generating {AI} symbol...")
    s2 = make_symbol("{AI}")

    print("4/4  Building SVGs...")
    p = portrait_svg(pg)
    sym1 = symbol_svg(s1)
    sym2 = symbol_svg(s2)
    ...
    for m in ("dark", "light"):
        ...
        with open(out, "w", encoding="utf-8") as f:
            f.write(build_svg(m, p, sym1, sym2))
